...
function PeriodosAquisitivosForm({
  ...
  // pronta (13º/Férias). Qualquer outra (Auxílio Alimentação/Transporte,
  // ou rubrica nova digitada livremente) cai na mini-calculadora abaixo.
  ...
  // Avos exibido na linha/campo: só faz sentido pras vantagens de
  // 13º/Férias (que têm fórmula pronta) — nas demais, fica "—"
  const avos = ehFormulaConhecida
    ? (selecionarVantagem.startsWith("13ª") ? avos13 : avosFerias)
    : null;

  // Determina se o Campo5 (Valor) é calculado automaticamente — por
  // fórmula pronta OU pela mini-calculadora — ou se fica livre para
  // digitação manual. Mantido com todas as casas decimais (sem
  // arredondar aqui) — o arredondamento só acontece na exibição.
        ...
        <tbody>
          {lista.map(item => (
            ...
            >
              ...
              <td style={{ padding: "8px", textAlign: "center" }}>{item.avos != null ? item.avos : "—"}</td>
